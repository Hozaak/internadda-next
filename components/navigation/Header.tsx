...
interface HeaderProps {
  // In Phase 6, we will connect this to your global state/Supabase auth
  user?: User | null; 
  onLogout?: () => void;
}
    ...
    >
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            {user ? (
                ...
                <AnimatePresence>
                  {isProfileOpen && (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, y: 10 }}
                      className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-[110]"
                    >
                      <Link href="/profile" className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                        <UserIcon size={18} className="text-indigo-600" /> View Profile
                      </Link>
                      <Link href="/settings" className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex items-center space-x-2">
                <Link href="/login" className="px-5 py-2 text-sm font-bold text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
                  Sign In
                </Link>
                <Link href="/signup" className="px-6 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 hover:-translate-y-0.5">
                  Get Started
                </Link>
              </div>
            )}
          ...
          {/* Pro Hamburger Animation Toggle */}
          <button
              ...
              <div className="p-6 border-t bg-slate-50/50">
                {user ? (
                  <div className="space-y-3">
                    <Link href="/profile" onClick={() => setMobileMenuOpen(false)} className="flex items-center space-x-3 p-4 bg-white border border-slate-200 rounded-xl text-slate-700 font-bold text-sm shadow-sm">
                      <UserIcon className="w-5 h-5 text-indigo-600" />
                      <span>My Profile</span>
                    </Link>
                    <button
                      onClick={onLogout}
                      className="w-full flex items-center justify-center space-x-2 py-4 text-red-600 rounded-xl font-bold text-sm border-2 border-red-50 border-dashed"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="py-3.5 text-center text-sm font-bold text-slate-700 bg-white border border-slate-200 rounded-xl shadow-sm">
                      Login
                    </Link>
                    <Link href="/signup" onClick={() => setMobileMenuOpen(false)} className="py-3.5 text-center text-sm font-bold text-white bg-indigo-600 rounded-xl shadow-lg">
                      Join Now
                    </Link>
                  </div>
                )}
